<?php require_once APP_ROOT . '/src/Views/Include/header.php'; ?>
    ...
    ?>
            <div class="col-12 col-md-6 border bg-light my-2">
                <a href="<?= URL_ROOT . '/blog/' . $post['slug']; ?>" class="text-body"><h2 class="display-3 mx-3 mb-0"><?= $post['title']; ?></h2></a>
                <small class="text-secondary border-left border-right border-secondary px-2">📅 <?= date("Y/m/d H:i", strtotime($post['updated_at'])); ?></small>
                <div class="media">
                    <?php
                    if ($imgSrc !== '') {
                    ?>
                    <img src=<?= $imgSrc ?> class="mr-3 border-right-0 border-secondary rounded-left leftMediaHalf" alt="<?= $post['title']; ?>">
                    <?php
                    }
                    ?>
                    <div class="media-body">
                        <hr class="mb-1 mt-0">
                        <p class="mb-2"><?= $post['subtitle']; ?>... <a href="<?= URL_ROOT . '/blog/' . $post['slug']; ?>" class="text-dark border border-dark rounded-pill pl-2 pr-0 linkButton">Read More 〉</a></p>
    ...
    ?>
            <div class="col-12 col-md-6 border bg-light my-2">
                <a href="<?= URL_ROOT . '/blog/' . $post['slug']; ?>" class="text-body"><h3 class="display-4 mx-2"><?= $post['title']; ?></h3></a>
                <div class="media">
                    <?php
                    if ($imgSrc !== '') {
                    ?>
